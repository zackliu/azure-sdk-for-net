...
namespace Azure.Messaging.WebPubSub.Client.Tests
{
    public class WebPubSubClientAckableOperationTests
    {
        ...
        public WebPubSubClientAckableOperationTests()
        {
        ...
        [Fact]
        public async Task SendMessageWithAckFailedTest()
        {
            var clientMoc = new Mock<WebPubSubClient>(new Uri("wss://test.com"), new WebPubSubClientOptions() { AutoReconnect = false });
            clientMoc.CallBase = true;
            clientMoc.Setup(c => c.SendCoreAsync(It.IsAny<ReadOnlyMemory<byte>>(), It.IsAny<WebPubSubProtocolMessageType>(), It.IsAny<bool>(), It.IsAny<CancellationToken>())).Returns(() =>
            ...
            var ex = await Assert.ThrowsAsync<SendMessageFailedException>(() => t).OrTimeout();
            Assert.Equal(1u, ex.AckId);
            Assert.Equal("error", ex.Error.Name);
            Assert.Equal("message", ex.Error.Message);
        }
        ...
        public async Task AckableOperationIntegrationTest_Failed()
        {
            ...
            await Assert.ThrowsAsync<SendMessageFailedException>(() => t).OrTimeout();
        }
        ...
        public async Task CleanAckableAfterConnectionClose()
        {
            ...
            await Assert.ThrowsAsync<SendMessageFailedException>(() => t1).OrTimeout();
            await Assert.ThrowsAsync<SendMessageFailedException>(() => t2).OrTimeout();
            await Assert.ThrowsAsync<SendMessageFailedException>(() => t3).OrTimeout();
        }
    }
}
