...
#pragma warning restore CA1816 // Dispose methods should call SuppressFinalize
        ...
        /// <param name="cancellationToken">An optional <see cref="CancellationToken" /> instance to signal the request to cancel the operation.</param>
        /// <returns>The ack for the operation.</returns>
        public virtual async Task<WebPubSubResult> JoinGroupAsync(string group, SyncAsyncEventHandler<WebPubSubGroupMessageEventArgs> handler, ulong? ackId = null, CancellationToken cancellationToken = default)
        {
            ThrowIfDisposed();
        ...
        /// </summary>
        public event SyncAsyncEventHandler<WebPubSubServerMessageEventArgs> ServerMessageReceived;

        /// <summary>
        /// A event triggered when received group data messages.
        /// </summary>
        public event SyncAsyncEventHandler<WebPubSubGroupMessageEventArgs> GroupMessageReceived;
        ...
        /// <returns></returns>
        private IDisposable GroupMessageReceivedInternal(string group, SyncAsyncEventHandler<WebPubSubGroupMessageEventArgs> handler)
        {
            var groupEntity = _groups.GetOrAdd(group, n => new WebPubSubGroup(n));
            groupEntity.MessageReceived += handler;
        ...
        private async Task SafeHandleGroupMessageAsync(GroupDataMessage message, CancellationToken token)
        {
            try
            {
                await GroupMessageReceived.RaiseAsync(new WebPubSubGroupMessageEventArgs(message, false, token), nameof(WebPubSubGroupMessageEventArgs), nameof(WebPubSubGroupMessageEventArgs)).ConfigureAwait(false);
            }
            catch
            {
            }
        ...
        private async Task SafeHandleServerMessageAsync(ServerDataMessage message, CancellationToken token)
        {
            try
            {
                await ServerMessageReceived.RaiseAsync(new WebPubSubServerMessageEventArgs(message, false, token), nameof(WebPubSubServerMessageEventArgs), nameof(WebPubSubServerMessageEventArgs)).ConfigureAwait(false);
            }
            catch
            {
            }
        ...
        private class SequenceId
        {
            ...
            public ulong Current => _sequenceId;
